...
def static_line(df, *args):
    df = compare(df, *args)
    ff = create_data(df)
    fig = px.line(ff, x="Date", y="Cases", color="Country",
                  template="plotly_dark", range_y=[0, ff["Cases"].max()])
...
def line_comparison(country):
    ...
    whole_df["confirmed"] = list(
        confirmed_global.loc[confirmed_global['country'] == country].values.flatten()[1:])
    whole_df["deaths"] = list(
        deaths_global.loc[deaths_global['country'] == country].values.flatten()[1:])
    whole_df["recovered"] = list(
        recovered_global.loc[recovered_global['country'] == country].values.flatten()[1:])
